...
{
	static public final Identifier ID = Identifier.fromNamespaceAndPath(StretchyLeashMod.MODID, "command");

	static private final String ROOT_COMMAND = StretchyLeashMod.MODID;
	...
	static public void RegisterWith(CommandDispatcher<CommandSourceStack> dispatcher, CommandBuildContext registryAccess, CommandSelection env){
		...
		root.then(config);
		root.requires(s -> s.permissions().hasPermission(new Permission.HasCommandLevel(PermissionLevel.GAMEMASTERS)));
