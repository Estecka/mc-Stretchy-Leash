...
{
	static public final Identifier ID = Identifier.of(StretchyLeashMod.MODID, "command");
	...
	static public void	RegisterWith(CommandDispatcher<ServerCommandSource> dispatcher, CommandRegistryAccess registryAccess, RegistrationEnvironment env){
		...
		root.then(config);
		root.requires(s -> s.getPermissions().hasPermission(new Permission.Level(PermissionLevel.GAMEMASTERS)));
